...
export default async function PinViewPage({ params }: { params: Promise<{ pin: string }> }) {
  ...
  const people = await getArchivePeople(data?.archive_id);
  const relationships = await getArchiveRelationships(data?.archive_id);
  const stories = await getArchiveStories(data?.archive_id);
