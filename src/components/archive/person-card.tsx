      ...
      <Card className="h-full overflow-hidden bg-[#f7f1e5] transition hover:-translate-y-0.5 hover:shadow-[0_22px_52px_rgba(0,0,0,0.25)]">
        <div className="relative h-44 bg-[#eadcc9]">
          {person.image_url ? (
            ...
            <div className="type-display grid h-full place-items-center text-[#8b4a2f]">
              {person.full_name.slice(0, 1)}
            </div>
          )}
        </div>
        <div className="p-5">
          <h3 className="type-h3">{person.full_name}</h3>
          <p className="type-caption mt-1 text-[#3a3029]/72">
            {person.birth_place || "Place not yet known"}
          </p>
          <p className="type-body-sm mt-4 line-clamp-3 text-[#2e2621]/90">
